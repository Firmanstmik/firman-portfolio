      ...
      <div className="section-pad container-max">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2">
          {PRINCIPLES.map((item, i) => {
            const copy = t.principles.items[i]
            return (
              <Reveal key={item.code} delay={i * 0.06}>
                <article className="card-surface group relative h-full overflow-hidden p-7 sm:p-8">
                  ...
                  <h3 className="mt-5 text-2xl font-semibold tracking-tight">{copy.title}</h3>
                  <p className="mt-3 max-w-md text-[0.95rem] leading-relaxed text-muted">{copy.desc}</p>
                </article>
              </Reveal>
            )
          })}
